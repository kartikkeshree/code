...
angular.module('users').controller('AuthenticationController', ['$scope', '$http', '$location', 'Authentication', 'Dentist','Staff',
	function($scope, $http, $location, Authentication, Dentist,Staff) {
			...
			$http.post('/auth/signin', $scope.credentials).success(function(response) {
				...
				if(response[0].role_id=="5"){ //if user type is user
					...
					$http.post('/dentistIdData',{user_id:response[0].id}).success(function(response1) { //To fetch Dentist Data
						...
						$scope.error = response1.message[0];
					});
					$location.path('/home');
                  ...
                  $http.post("/getModulesAccessByStaff",{id:response[0].id}).success(function(response2){
                      ...
                      $scope.error = response2.message[0];
                  })
                  $location.path('/home');
